...
def news_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if  __isAdmin__ == "是" and 'news' != 'forum' :
            if req_dict.get("userid") and 'news' != 'chat' and 'news' != 'examrecord':
                del req_dict["userid"]
        else:
...
def news_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        idOrErr= news.createbyreq(news,news, req_dict)
...
def news_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if where:
            ...
            sql = f'''select * from news where typename in ('{"','".join(leixing)}') union all select * from news where typename not in('{"','".join(leixing)}')'''
        cursor.execute(sql)
...
def news_security(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT * FROM news where ='{0}'".format(req_dict['username'])
        record = {}
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
