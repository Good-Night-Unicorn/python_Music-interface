...
def yonghu_register(request):
    if request.method in ["POST", "GET"]:
        ...
        if "emailcode" in req_dict:
            emailcode = req_dict['emailcode']
            del req_dict['emailcode']

            cursor = connection.cursor()
            cursor.execute("select * from yonghu where email='"+req_dict['email']+"' limit 1")
            raw = cursor.fetchone()
            if raw != None:
            ...
            cursor.execute("select code from emailregistercode where email='"+req_dict['email']+"' order by addtime desc limit 1")
            raw = cursor.fetchone()
           
            if raw != None and emailcode != raw[0]:
                msg['code'] = crud_error_code
                msg['msg'] = "邮箱验证码不正确"
...
def yonghu_login(request):
    if request.method in ["POST", "GET"]:
        ...
        params = {yonghu.__loginUser__:username}
        userList = yonghu.getbyparams(yonghu, yonghu, params)
...
def yonghu_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if  __isAdmin__ == "是" and 'yonghu' != 'forum' :
            if req_dict.get("userid") and 'yonghu' != 'chat' and 'yonghu' != 'examrecord':
                del req_dict["userid"]
        else:
...
def yonghu_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if yonghu.objects.filter(nicheng = req_dict['nicheng']).count()>0:
            msg['code'] = crud_error_code
            msg['msg'] = "昵称已存在"
            return JsonResponse(msg, encoder=CustomJsonEncoder)
        tablename=request.session.get("tablename")
...
def yonghu_update(request):
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
        if yonghu.objects.exclude(id=req_dict['id']).filter(nicheng = req_dict['nicheng']).count()>0:
            msg['code'] = crud_error_code
            msg['msg'] = "昵称已存在"
...
def yonghu_value(request, xColumnName, yColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        ...
        sql = ''
        if timeStatType == '日':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, ROUND(sum({1}),2) total FROM yonghu {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d')".format(xColumnName, yColumnName, where, '%Y-%m-%d')

        if timeStatType == '月':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, ROUND(sum({1}),2) total FROM yonghu {2} GROUP BY DATE_FORMAT({0}, '%Y-%m')".format(xColumnName, yColumnName, where, '%Y-%m')

        if timeStatType == '年':
            sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, ROUND(sum({1}),2) total FROM yonghu {2} GROUP BY DATE_FORMAT({0}, '%Y')".format(xColumnName, yColumnName, where, '%Y')
...
def yonghu_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, ROUND(sum({1}),2) AS total FROM yonghu {2} GROUP BY {0}".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def yonghu_valueMul(request, xColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        ...
        for item in req_dict['yColumnNameMul'].split(','):
            sql = ''
            if timeStatType == '日':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, ROUND(sum({1}),2) total FROM yonghu {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d') LIMIT 10".format(xColumnName, item, where, '%Y-%m-%d')

            if timeStatType == '月':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, ROUND(sum({1}),2) total FROM yonghu {2} GROUP BY DATE_FORMAT({0}, '%Y-%m') LIMIT 10".format(xColumnName, item, where, '%Y-%m')

            if timeStatType == '年':
                sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, ROUND(sum({1}),2) total FROM yonghu {2} GROUP BY DATE_FORMAT({0}, '%Y') LIMIT 10".format(xColumnName, item, where, '%Y')
...
def yonghu_o_valueMul(request, xColumnName):
    if request.method in ["POST", "GET"]:
        ...
        where = ' where 1 = 1 '
        for item in req_dict['yColumnNameMul'].split(','):
            sql = "SELECT {0}, ROUND(sum({1}),2) AS total FROM yonghu {2} GROUP BY {0} LIMIT 10".format(xColumnName, item, where)
            L = []
            cursor = connection.cursor()
            cursor.execute(sql)
            desc = cursor.description
...
def yonghu_shBatch(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        for id in req_dict.get("ids"):
            data =yonghu.getbyid(yonghu,yonghu, int(id))
            if len(data)>0:
                data=data[0]
                data['sfsh'] = req_dict.get("sfsh")
                data['shhf'] = req_dict.get("shhf")
                yonghu.updatebyparams(yonghu, yonghu, data)
