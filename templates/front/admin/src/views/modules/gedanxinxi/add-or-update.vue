	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="专辑名称" prop="zhuanjimingcheng" >
					<el-input v-model="ruleForm.zhuanjimingcheng" placeholder="专辑名称" clearable  :readonly="ro.zhuanjimingcheng"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="专辑名称" prop="zhuanjimingcheng" >
				...
				<el-form-item class="input" v-if="type!='info'"  label="创建人" prop="nicheng" >
					<el-input v-model="ruleForm.nicheng" placeholder="创建人" clearable  :readonly="ro.nicheng"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="创建人" prop="nicheng" >
					<el-input v-model="ruleForm.nicheng" placeholder="创建人" readonly></el-input>
				</el-form-item>
			</template>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `gedanxinxi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.gedanxinxiCrossAddOrUpdateFlag = false;
											this.parent.search();
