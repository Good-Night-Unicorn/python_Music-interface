		...
		<div class="list-preview">
			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<el-form-item class="list-item">
					<div class="lable">歌名：</div>
					<el-input v-model="formSearch.songname" placeholder="歌名" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
					<div class="lable">专辑名称：</div>
					<el-input v-model="formSearch.zhuanjimingcheng" placeholder="专辑名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-button class="list-search-btn" v-if=" true " type="primary" @click="getList(1, curFenlei)">
					查询
				</el-button>
				<el-button class="list-add-btn" v-if="btnAuth('gedanxinxi','新增')" type="primary" @click="add('/index/gedanxinxiAdd')">
					添加
				</el-button>
			</el-form>
			<div class="select2">
			...
			<div class="sort_view">
				<el-button class="click-sort-btn" @click="sortClick('clicknum')">
					...
					<span class="icon iconfont icon-jiantou06" v-else-if="sortType=='clicknum'&&sortOrder=='desc'"></span>
					<span class="icon iconfont icon-jiantou07" v-else-if="sortType=='clicknum'&&sortOrder=='asc'"></span>
				...
				<el-button class="like-sort-btn" @click="sortClick('thumbsupnum')">
					<span class="icon iconfont icon-zan07" v-if="sortType!='thumbsupnum'"></span>
					<span class="icon iconfont icon-jiantou06" v-else-if="sortType=='thumbsupnum'&&sortOrder=='desc'"></span>
					<span class="icon iconfont icon-jiantou07" v-else-if="sortType=='thumbsupnum'&&sortOrder=='asc'"></span>
			...
			<div class="list">
				<div class="list6">
					<div v-for="(item,index) in dataList" :key="index" :class="'list-item' + ((index%2) + 1)" @click.stop="toDetail(item)">
						<div class="imgbox">
							<img class="playImg" @click.stop="playClick(item)" src="../../assets/play.png" alt="">
...
<script>
	export default {
		...
		methods: {
			...
			async getFenlei() {
				await this.$http.get('yinleleixing/list',{params: {sort: 'id',order: 'asc'}}).then(res => {
					if (res.data.code == 0) {
						this.fenlei = res.data.data.list
					}
				});
				this.feileiColumn = 'yinleleixing'
			},
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.zhuanjimingcheng != '') searchWhere.zhuanjimingcheng = '%' + this.formSearch.zhuanjimingcheng + '%';
				if(!this.centerType){
					params['sfsh'] = '是';
				}
				if (this.curFenlei != '全部') searchWhere.yinleleixing = this.curFenlei;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`gedanxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/gedanxinxiDetail', query: params});
			},
			playClick(item) {
				...
				for(let i in this.dataList){
					if(this.changeAudio(this.dataList[i].id)){
						this.$store.state.app.audio.push({
							...
							lrc: this.changeBian(this.dataList[i].lyric),
						})
